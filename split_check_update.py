        ...
        if my_sql_df_control_date.loc[index]['market'] == "SPB":
            if us_stock.isin([my_sql_df_control_date.loc[index]['st_id']]).any():
                try:
                    ...
                    save_exeption_log(linux_path, modul='split_check: find_slit: investpy', message=str(_ex))
                ...
                try:
                    ...
                    save_exeption_log(linux_path, modul='split_check: find_slit: Yahho', message=str(_ex))
...
def insert_history_date_into_sql():
    ...
    for only_us_index in tqdm(list_stock_from_real_us_to_us):
        ...
        try:
            ...
            save_exeption_log(linux_path, modul='insert US: to SQL: investpy', message=str(_ex))
            continue
