...
def stik_up_status(linux_path):
    global stiker_load_status
    n = 11
    if n == 1:
        stiker_name = pd.read_excel(linux_path + 'my_test_all_ru-spb.xlsx', index_col=0)

        st_up_stat['stik_upload_status'] = 'No data'
        stiker_load_status = stiker_load_status.join(pd.DataFrame(st_up_stat, columns=['stik_upload_status']))
    elif n == 11:
